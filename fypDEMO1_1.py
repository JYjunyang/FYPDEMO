...
def TriggerCleaning():
    ...
    # for i in range(len(dfOri['full_text'].values.tolist())):
    #     print(dfOri['full_text'][i])
...
def FeaturesExtraction():
    ...
    for index, row in dfOri.iterrows():
        ...
        document = [gensim.models.doc2vec.TaggedDocument(words=row['full_text'],tags=[index])]
    b1 = time.time()
...
#     sentenceAveVec = get_ave_vector(word2vec)
#     print(sentenceAveVec)
#     #splitting data
#     X_trainval, X_test, y_trainval, y_test = train_test_split(sentenceAveVec,dfOri['Sentiment'].values.tolist(),
#                                                               random_state=0)
